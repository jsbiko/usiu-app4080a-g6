...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ================================
  // 0. ACTIVE NAVIGATION HIGHLIGHTING
  // ================================
  ...
  window.addEventListener("scroll", () => {
    if (hero && heroContent) {
      ...
      if (scrollY < fadePoint) {
        ...
        heroContent.style.transform = `translateY(${Math.min(
          14,
          scrollY * 0.12
        )}px)`; // gentle shift only
      } else {
  ...
  // ================================
  document
    .querySelectorAll(".social-btn, .btn-primary, .btn-outline")
    .forEach((btn) => {
...
if (heroImg) {
  ...
  function animateParallax() {
    ...
    if (window.innerWidth > 960) {
      heroImg.style.transform = `translate(${targetX}px, ${
        targetY * 0.2
      }px) scale(1.03)`;
      // Optional: dynamic shadow intensity based on mouse movement
      function updateShadow() {
        const intensity = Math.min(0.4, Math.abs(mouseX) / 60);
        heroImg.style.filter = `drop-shadow(${mouseX * 0.2}px ${
          mouseY * 0.2
        }px 20px rgba(30,58,138,${intensity}))`;
